...
public class CustomProcessWindowFunction extends ProcessWindowFunction<ClusterReading, String, String, TimeWindow> {

    @Override
    public void process(String type, ProcessWindowFunction<ClusterReading, String, String, TimeWindow>.Context context, Iterable<ClusterReading> iterable, Collector<String> collector) throws Exception {
        // 统计数据的数量
        int count = 0;
        // 增强for循环遍历全窗口所有数据
        for (ClusterReading cluster : iterable) {
            count += cluster.getAlpha();
        }
        ...
        int i = 0; // 计数
        for (ClusterReading cluster : iterable) {
            while (cluster.getAlpha() > 0) {
                data[i] = new double[]{cluster.getX(), cluster.getY()};
                cluster.setAlpha(cluster.getAlpha() - 1);
                i++;
            }
        }
...
//        String resultStr = "id:" + integer + "聚类中心为：" + "x=" + data[Math.toIntExact(centersmax)][0] + "y=" + data[Math.toIntExact(centersmax)][1];
        ...
        for (long center : centers) {
            centersb.append("(x: " + data[Math.toIntExact(center)][0] + ",y: " + data[Math.toIntExact(center)][1] + ",alpha: " + (int) (center + 1) + "),");
        }
